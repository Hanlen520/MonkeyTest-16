...
def start(dev):
    ...
    ##根据个人运行环境不同，如果出现异常“ADB server didn't ACK”时，可执行line69~line71该注释部分解决
...
if __name__ == '__main__':
    #根据个人运行环境不同，如果出现异常“error: could not install smartsocket listener: cannot bind to 127.0.0.1:5037: 通常每个套接字地址(协议/网络地址/端口)只允许使用一次”时，可执行该注释部分解决
    """path_bat= "D:\\PyCharm\\Monkey_performance\\" #批处理文件路径
    os.system('start ' + os.path.join(path_bat, 'kill_port.bat')) #批处理关闭5037端口
    #重启adb服务
    os.popen("adb kill-server adb")
    os.popen("adb start-server")
    time.sleep(10)"""
