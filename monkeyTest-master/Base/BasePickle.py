...
def readInfo(path):
    l = list(path)
    # 修改打开文件名称不对应(特殊符号)  debug1.testin.cn 远程统一名称  127.0.0.1
    if len(l) > 39 and 'testin.cn' in path:
        # 43 127.0.0.1
        l[50] = '_'
    # elif len(l) <31 and '127.0.0.1' in path :
    #      l[9] = '_'  # 修改字符串的第1个字符为z
    path = ''.join(l)
    data = []
    print(path)
